<script setup lang="ts">
import { computed } from 'vue'
import { storeToRefs } from 'pinia'
import { useQueryBuilderStore } from '@/stores/query-builder'
import SqlPreview from './SqlPreview.vue'
import QueryInfo from './QueryInfo.vue'

const queryBuilderStore = useQueryBuilderStore()

const generatedSql = computed(() => queryBuilderStore.generatedSql)
const queryInfo = computed(() => queryBuilderStore.queryInfo)
const analysisResult = computed(() => queryBuilderStore.analysisResult)
const { smartQuote } = storeToRefs(queryBuilderStore) // Pinia state to ref
const { t } = useI18n()
const toast = useToast()

const copyToClipboard = async () => {
  try {
    await navigator.clipboard.writeText(generatedSql.value)
    toast.add({
      title: t('queryBuilder.rightPanel.copied'),
      color: 'primary',
      icon: 'i-heroicons-check-circle'
    })
  } catch (err) {
    console.error('Failed to copy:', err)
  }
}
</script>

<template>
  <div class="flex flex-col h-full bg-white dark:bg-gray-900">
    <!-- SQLプレビュー -->
    <div class="flex flex-col flex-1 min-h-[200px] overflow-hidden">
      <div class="flex items-center justify-between px-3 py-2 border-b border-gray-200 dark:border-gray-800">
        <span class="text-sm font-medium">{{ t('queryBuilder.rightPanel.title') }}</span>
        <div class="flex items-center gap-3">
          <div class="flex items-center gap-1.5" :title="t('queryBuilder.rightPanel.smartQuote')">
            <USwitch
              :model-value="smartQuote"
              @update:model-value="(val) => queryBuilderStore.setSmartQuote(val)"
              size="sm"
            />
            <span class="text-xs text-gray-500 dark:text-gray-400">{{ t('queryBuilder.rightPanel.smartQuote') }}</span>
          </div>
          <UButton
            icon="i-heroicons-clipboard-document"
            size="xs"
            color="neutral"
            variant="ghost"
            :title="t('queryBuilder.rightPanel.copy')"
            @click="copyToClipboard"
          />
        </div>
      </div>
      <SqlPreview
        :sql="generatedSql"
        :error-details="queryBuilderStore.queryError?.details"
        :analysis-result="analysisResult"
      />
    </div>

    <!-- クエリ情報 -->
    <div class="flex-shrink-0 border-t border-gray-200 dark:border-gray-800">
      <div class="flex items-center justify-between px-3 py-2 border-b border-gray-200 dark:border-gray-800">
        <span class="text-sm font-medium">{{ t('queryBuilder.rightPanel.infoTitle') }}</span>
      </div>
      <QueryInfo :info="queryInfo" />
    </div>
  </div>
</template>
