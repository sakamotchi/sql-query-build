...
const syncWindowTitle = async () => {
  ...
  try {
    ...
    console.warn('[MutationBuilder] Failed to set window title:', error)
  }
}
...
watch(
  ...
  () => {
    if (resolvedConnection.value) {
      ...
      pageTitle.value = `${resolvedConnection.value.name} [${envLabel}] - ${t('mutationBuilder.pageTitle')}`
    } else {
      pageTitle.value = t('mutationBuilder.pageTitle')
    }
