# FromSimpleDialog 詳細設計

## 概要
Simpleプロバイダーから Master Password への移行を担当する専用ダイアログ。

**初期実装スコープ:** Simple → Master Password のみ
**将来実装予定:** Simple → Keychain
...
- バックエンドからのエラーは `errorMessage` に格納
- `currentPhase` を `'error'` に設定
- エラーフェーズでは「やり直す」ボタンを表示し、`reset()`を実行

## テストケース (初期実装)

### 1. Simple → Master Password (正常系)
1. confirm フェーズで「続行」をクリック → initialize へ
2. パスワード8文字未満で入力 → ボタン無効化
3. パスワード8文字以上、確認パスワード不一致 → ボタン無効化
4. パスワード8文字以上、確認パスワード一致 → ボタン有効化
5. 「変更」をクリック → switching → complete → ダイアログ閉じる

### 2. エラーハンドリング
1. バックエンドエラー発生 → error フェーズへ
2. 「やり直す」クリック → confirm フェーズへリセット
...
2. **nextPhase の分岐追加**
   ```typescript
   case 'confirm':
     if (props.targetProvider === 'keychain') {
       currentPhase.value = 'switching'
       await nextTick()
       await switchProvider()
     } else {
...
3. **テンプレートの条件追加**
   ```vue
   <div v-if="currentPhase === 'initialize' && props.targetProvider === 'master-password'">
   ```
