...
- Simpleプロバイダーは、CredentialStorageが初回にキーを取得する際に未初期化であれば自動で初期化する。
- ConnectionService側では接続情報を返す際にパスワードを常に除去し、保存はCredentialStorage経由に統一。既存データに旧形式の暗号化パスワードが残っている場合は MasterKeyManager で復号し、可能であればCredentialStorageへ移行する。
