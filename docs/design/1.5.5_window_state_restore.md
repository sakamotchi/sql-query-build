...
- 保存時はウィンドウの位置・サイズをディスプレイのスケールファクターで論理座標に変換する
- 復元時は現在のモニターの論理解像度にクランプし、異常な縦長・はみ出しを防ぐ
- ウィンドウを閉じた場合は、そのラベルの状態をストレージから削除し、次回復元対象に含めない
- ウィンドウ生成直後にも現在の状態を保存し、強制終了でも復元できる初期データを残す
...
**ファイル**: `src/App.vue`（このプロジェクトではルーター未使用のため、パスでランチャー/クエリビルダーを出し分け）
