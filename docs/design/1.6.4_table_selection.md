...
DB構造ツリーからテーブルを中央パネルのテーブル関係図エリアにドラッグ＆ドロップして、クエリで使用するテーブルを選択する機能を実装する。選択されたテーブルはカード形式で表示され、複数テーブルの選択をサポートする。
...
/**
 * ダブルクリックでテーブル追加（DB構造ツリーから）
 */
const handleTableDoubleClick = (table: Table) => {
  addTable(table);
};

// 外部からテーブル追加できるようにエクスポート
defineExpose({
  addTable,
