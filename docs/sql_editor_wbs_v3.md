...
**目的**: モーダルUIでJOIN設定を可能にする

| タスクID | タスク名 | 依存関係 | 完了条件 |
|---------|---------|---------|---------|
| 6A.1 | JoinConfigDialog.vue | Phase 2 | JOIN種別/条件設定ダイアログ |
| 6A.2 | JoinConditionRow.vue | 6A.1 | ON条件の設定行コンポーネント |
| 6A.3 | JOIN設定ストア統合 | 6A.2 | query-builderストアとの連携 |
| 6A.4 | JOIN SQL生成機能拡張 | 6A.3 | SQL生成エンジンのJOIN対応 |

**成果物**:
- JoinConfigDialog（モーダルベース）
- JoinConditionRow（条件設定UI）
- JOIN設定の保存・復元機能
...
**次のアクション**: Phase 5エクスポート機能、またはPhase 6A JOIN設定UI拡張（基本機能）の開始を検討
