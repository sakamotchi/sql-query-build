...
**概要**: クエリ実行時の復号化処理がボトルネックとなっているため、復号化済みパスワードのセッションキャッシュを実装する。
...
| タスクID | タスク名 | 依存関係 | 完了条件 |
|---------|---------|---------|---------|
| 7.2.1 | 復号化キャッシュ設計 | Phase 2 | キャッシュ戦略・有効期限・セキュリティ考慮の設計完了 |
| 7.2.2 | DecryptedPasswordCache実装 | 7.2.1 | メモリキャッシュ機構の実装完了 |
| 7.2.3 | ConnectionService統合 | 7.2.2 | get_by_id()でキャッシュを利用するよう変更 |
| 7.2.4 | キャッシュ無効化処理 | 7.2.3 | 接続情報更新時・アプリ終了時のキャッシュクリア |
| 7.2.5 | テスト・検証 | 7.2.4 | パフォーマンス改善の計測・セキュリティ確認 |
...
**セキュリティ考慮事項**:
- メモリに平文パスワードを保持する期間を最小限に
- セッション有効期限の設定（例: 5分）
- アプリ終了時・接続情報更新時の確実なキャッシュクリア
- マスターパスワード変更時のキャッシュ無効化
