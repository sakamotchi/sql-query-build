...
4. **既存機能との統合**: クエリ保存・履歴機能、WhereTab構成の再利用により、開発効率と一貫性を向上
...
- **受け入れ条件**:
  ...
  - [ ] MutationWhereTab.vueでWhereTab構成を再利用してWHERE条件を設定できる
...
- **保守性**:
  ...
  - MutationWhereTab.vueでWhereTab構成を再利用し、コードの重複を避ける
...
- テーブル選択・SET句設定UI
- WHERE条件設定UI（MutationWhereTab.vueでWhereTab構成を再利用）
...
- `app/components/mutation-builder/MutationWhereTab.vue` - WHERE条件設定タブ（WhereTab構成を再利用）
...
- **Phase 1.6（クエリビルダー基盤）**: DatabaseTree、WhereTab.vue構成を再利用
